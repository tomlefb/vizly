      ...
      >
        <div className="mx-auto max-w-6xl px-4 py-8 md:px-6 md:py-12 lg:px-8">
          ...
          >
            ...
            {/* ====== PROJECTS as bento widgets ====== */}
            {sortedProjects.map((project, index) => {
              // Vary widget sizes: first project gets 2x2, rest alternate 2x1 and 1x1
              const isHero = index === 0
              const isWide = index > 0 && index % 3 === 1

              const colSpan = isHero ? 4 : isWide ? 2 : 2
              const showLargeImage = isHero

              return (
                <article
                  key={project.id}
                  className={`col-span-${colSpan >= 4 ? '4' : '2'} ${
                    isHero ? 'md:col-span-4' : 'md:col-span-2'
                  }`}
                  style={{
                    backgroundColor: cardBg,
                    borderRadius: 20,
