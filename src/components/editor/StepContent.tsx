...
import type { KpiItem, KpiType } from '@/types/kpis'
...
const textareaClass =
  'w-full rounded-lg border border-[#E5E7EB] bg-white px-3 py-2 text-sm text-[#111827] placeholder:text-[#9CA3AF] transition-[border-color] duration-150 resize-y min-h-[72px] focus:outline-none focus:border-[#D1D5DB] focus:shadow-[0_0_0_3px_rgba(0,0,0,0.04)]'
...
export function StepContent({
  ...
  const closeModal = useCallback(() => {
    setModalType(null); setEditingIndex(-1); setLocalFiles([]); setIsUploadingImages(false)
  }, [])
  ...
  const openAddProject = useCallback(() => {
    setEditingProject({ ...EMPTY_PROJECT, display_order: projects.length }); setLocalFiles([]); setEditingIndex(-1); setModalType('project')
  }, [projects.length])

  const openEditProject = useCallback((index: number) => {
    const p = projects[index]; if (!p) return
    setEditingProject({ ...p }); setLocalFiles([]); setEditingIndex(index); setModalType('project')
  }, [projects])
        ...
        <div className="flex-1 mt-8 lg:mt-0 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <div className="relative" ref={dropdownRef}>
              <button type="button" onClick={() => setShowDropdown(!showDropdown)} className="inline-flex items-center gap-1.5 text-sm font-medium text-[#E8553D] hover:text-[#D4442E] transition-colors">
      ...
      {modalType && createPortal(
        <div className="fixed inset-0 z-[100] flex items-center justify-center">
          ...
          <div className={cn('relative w-full max-h-[85vh] flex flex-col bg-white rounded-xl overflow-hidden mx-4', modalType === 'layout' ? 'max-w-[960px]' : 'max-w-[560px]')}>
                ...
                <>
                  ...
                  <div>
                    ...
                    <div className="rounded-xl border border-[#E5E7EB] bg-[#F9FAFB] p-6 flex items-center justify-center min-h-[100px]">
                      <KpiRenderer kpi={editingKpi} primaryColor="#E8553D" />
                    </div>
                  </div>
                </>
              )}
            ...
            {/* Sticky footer */}
            {modalType === 'project' && <ModalFooter onSave={saveProject} disabled={!editingProject.title.trim()} label={editingIndex >= 0 ? 'Enregistrer' : 'Ajouter'} />}
